...
impl Almanac {
    ...
    pub fn init_seed_range(&self) -> Option<(u64, u64)> {
        // will store tuples that are at a minimum
        let mut candidates: Vec<(u64, u64)> = Vec::new();

        // iterate through each of the seeds
        for chunk in self.init_seeds.chunks(2) {
            let mut chunk_tuples = vec![(chunk[0], chunk[1])];
            // process each map
            chunk_tuples = self.apply_map(chunk_tuples, &self.seed_soil);
            chunk_tuples = self.apply_map(chunk_tuples, &self.soil_fert);
            chunk_tuples = self.apply_map(chunk_tuples, &self.fert_water);
            chunk_tuples = self.apply_map(chunk_tuples, &self.water_light);
            chunk_tuples = self.apply_map(chunk_tuples, &self.light_temp);
            chunk_tuples = self.apply_map(chunk_tuples, &self.temp_humid);
            chunk_tuples = self.apply_map(chunk_tuples, &self.humid_loc);
            if let Some(tup) = chunk_tuples.iter().min_by_key(|x| x.0) {
                candidates.push(tup.clone().clone());
            }
        }
        candidates.iter().min_by_key(|x| x.0).copied()
    }

    fn apply_map(&self, mut chunk_tuples: Vec<(u64, u64)>, map: &AlMap) -> Vec<(u64, u64)> {
        let mut answer = vec![];

        for rng in &map.ranges {
            let src = rng.source_start;
            let src_end = rng.source_start + rng.range_len as u64;
            let mut continued_candidate = vec![];

            while let Some(tuple) = chunk_tuples.pop() {
                let st = tuple.0;
                let ed = tuple.1;
                ...
                if before.1 > before.0 {
                    continued_candidate.push(before);
                }
                if inter.1 > inter.0 {
                    answer.push((
                        inter.0 - rng.source_start + rng.dest_start,
                        inter.1 - rng.source_start + rng.dest_start,
                    ));
                }
                if after.1 > after.0 {
                    continued_candidate.push(after);
                }
            }
            println!("{continued_candidate:?}");
            chunk_tuples = continued_candidate;
        }

        chunk_tuples.append(&mut answer);
        chunk_tuples
    }
}
